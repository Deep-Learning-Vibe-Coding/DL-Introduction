...
# Find the code cell and update the image path
for cell in notebook['cells']:
    if cell['cell_type'] == 'code':
        source = ''.join(cell['source'])
        if 'test_foot_data' in source:
            # Update the image path to the new filename
            new_code = """import cv2
import numpy as np
...
# 이미지 경로 (간단한 파일명으로 변경)
image_path = 'test_foot_data/foot_left.png'
...
if original_image is None:
    ...
    if os.path.exists('test_foot_data'):
        print("\\ntest_foot_data 디렉토리 내용:")
        files = os.listdir('test_foot_data')
        for f in files:
            print(f"  - {f}")
    else:
...
print("✓ 노트북이 수정되었습니다!")
print("✓ 새 이미지 경로: test_foot_data/foot_left.png")
print("✓ 이미지 파일이 생성되어 있습니다.")
print("\\n이제 Jupyter Notebook을 다시 실행하세요!")
