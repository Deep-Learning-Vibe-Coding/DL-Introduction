...
# New processing code (uses PIL for robust image loading)
new_code = """import cv2
import numpy as np
...
# 이미지 로드 (PIL 사용 → numpy 배열)
try:
    pil_img = Image.open(image_path)
    original_image = np.array(pil_img)
    # OpenCV uses BGR, Pillow gives RGB; convert to BGR for consistency with later code
...
# Replace the first code cell containing 'import cv2'
...
print('✅ notebook updated: image loading now uses Pillow for robust Unicode path handling.')
